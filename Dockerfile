...
CMD ["sh", "-c", "python manage.py makemigrations orders && python manage.py migrate && python manage.py collectstatic && gunicorn savannah.wsgi:application --bind 0.0.0.0:8000"]