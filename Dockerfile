...
CMD ["sh", "-c", "python manage.py makemigrations && python manage.py makemigrations orders && python manage.py migrate && python scripts/startup.py && gunicorn savannah.wsgi:application --bind 0.0.0.0:8000"]
